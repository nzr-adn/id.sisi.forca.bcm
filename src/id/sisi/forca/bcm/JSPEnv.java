...
public class JSPEnv {
	...
	public static Properties getCtx(HttpServletRequest request) {
		...
		Properties ctx = (Properties) session.getAttribute(WebSessionCtx);
		if (ctx == null) {
			ctx = new Properties();
			ctx = getDefault();
		}
		session.setAttribute(WebSessionCtx, ctx);

		WebUser wu = WebUser.get(request);
		if (wu != null) {
			int AD_User_ID = wu.getAD_User_ID();
			Env.setContext(ctx, Env.AD_USER_ID, AD_User_ID); // security
		}
